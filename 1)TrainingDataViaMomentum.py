#This code will use basic SGD to train our model using the gradients we deifned in the previos chapter. 
import numpy as np
import nnfs
...
class Optimizer_SGD():
    ...
    def Params_update(self, layer):
        ...
        if self.momentum:
            # if the layer does not contain momentun arrays, create them now
            if not hasattr(layer, 'weights_momentum'):
                #make an array with zeros j like w and b arrays
                layer.weights_momentum = np.zeros_like(layer.weights)

                #if there is no array forweights then there fs isnt one for biases eithetr so...
                layer.biases_momentum = np.zeros_like(layer.biases)

            #build weight updtes using momentum. Take the previous updates of weights or biases and multiplies with momentum(retain factor) and then updates the weighrs based on that
            weight_updates = self.momentum * layer.weights_momentum - (self.current_Learning_rate*layer.dweights)

            #and this line of code comes next. which is why the momentum reflects the value from the previous loop
            layer.weights_momentum = weight_updates

            #same thing with bias
            bias_updates = self.momentum * layer.biases_momentum - (self.current_Learning_rate*layer.dbiases)
            layer.biases_momentum = bias_updates

        #vanilla SGD uupdates if not using momentum
        else:
        ...
        #lets updates the weights and biases using either of these 
