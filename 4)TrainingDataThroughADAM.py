...
class Optimizer_AdaM():
    ...
    def __init__(self, learning_rate = 0.02, decay = 0, Beta1 = 0.9, beta2 = 0.999, epsilon = 1e-7):
    ...
    def Params_update(self, layer):
        ...
        #this is to offset the zero values the biases have to start with 
        #iterations + 1 because we are running on the iteration number of the previous loop and new one updates post update 
        # we also just start with iteration 0 instead of one so yeah 
        weight_momentums_corrected  = layer.weights_momentum / (1-(self.beta1**(self.iterations+1)))
        biases_momentums_corrected  = layer.biases_momentum / (1-(self.beta1**(self.iterations+1)))
        ...
        #do the same with these to get the current cache
        weight_cache_corrected = layer.weights_cache/(1-(self.beta2)**(self.iterations+1))
        biases_cache_corrected = layer.biases_cache/(1-(self.beta2)**(self.iterations+1))


        #vanilla update and square rooth cacche based normalization
